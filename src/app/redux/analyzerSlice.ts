...
import {
  ...
  analyzeSearch,
  analyzeUrls,
} from '../analyzer/utils/analyzeHelpers';
...
>('analyzer/analyzeData', async (authData, { getState, dispatch }) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(analyzeData.rejected, (state, action) => {
        state.loading = false;
        state.loadingMessage =
          action.error.message || 'An error occurred during analysis.';
      })
      ...
      .addCase(handleAnalyzeMore.rejected, (state, action) => {
        state.loading = false;
        state.loadingMessage =
          action.error.message || 'An error occurred while loading more data.';
      });
  },
});
